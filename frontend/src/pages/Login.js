...
class Login extends Component {
  ...
  handleSubmit = (event) => {
    ...
    if (Object.values(this.state)?.includes(null) || Object.values(this.state)?.includes('')) {
      alert('Please enter all credentials!')
      return false;
    } else {
      fetch(`http://localhost:5000/api/login?username=${this.state.username}&password=${this.state.password}`, {
        method: "GET",
        credentials: "include",
        headers: {
    ...
    return (
      <div className="login" >
        ...
        <div className="wrapper">
          ...
          <div className="right">
            <Form>
              <input onChange={this.handleInputChange} type="text" placeholder="Username" name="username" />
